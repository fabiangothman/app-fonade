...
abstract class DAOCalendario{
  	...
  	static public function getIntrVisitas($intr_id){
  		$query = 'SELECT vis.id,
  		...
  		WHERE
  		LOWER(rol_empr.nombre_unico)="emprendedor" AND LOWER(rol_intr.nombre_unico)="interventor" AND LOWER(rol_lae.nombre_unico)="lae" AND intr.lae_id='.$intr_id;
  		//var_dump($query);
  		$result = self::$conexion->query_db($query);
  		if(isset($result[0]))
			return $result;
...
class calendarioCRUD{
	...
	public function getIntrVisitas($intr_id){
		return DAOCalendario::getIntrVisitas($intr_id);
	}
