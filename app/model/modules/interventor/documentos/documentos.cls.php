...
abstract class DAODocumentos{
  	...
  	static public function getIntrAndEmprDocumentos($intr_id){
  		...
  		$queryIntrDocs = 'SELECT doc.*, intr.nombres, intr.apellidos FROM '._DBPFX_.'documento doc
  	...
  	static public function getUsuariosIntrAndEmpr($intr_id){
  		$queryEmprUsers = 'SELECT empr.id, empr.nombres, empr.apellidos FROM '._DBPFX_.'usuario empr
  		INNER JOIN '._DBPFX_.'proyecto pry ON pry.emprendedor_id=empr.id
  		INNER JOIN '._DBPFX_.'rol rol_emp ON rol_emp.id=empr.rol_id
  		WHERE LOWER(rol_emp.nombre_unico)="emprendedor" AND pry.interventor_id='.$intr_id;
...
class documentosCRUD{
	...
	public function getIntrAndEmprDocumentos($intr_id){
		return DAODocumentos::getIntrAndEmprDocumentos($intr_id);
	}
	public function getUsuariosIntrAndEmpr($intr_id){
		return DAODocumentos::getUsuariosIntrAndEmpr($intr_id);
	}
