	...
	****************************************************************************/
	...
	class emprendedores extends controller
	{
		...
		protected function index()
		{
			...
			$this->listaCiudades = $emprendedores->getCiudades();
			$this->listaRoles = $emprendedores->getRoles();

			$this->default_pw = _USER_DEFAULT_PASSW_;

			$this->addInReadyCode("
				MSEmprendedores.init('"._MSFW_PATH_."', '"._MODEL_PATH_."', '"._VIEW_PATH_."', ".json_encode($this->listaCiudades).", ".json_encode($this->listaRoles).", '".$this->rol."');
				//
			");
		}
