	...
	****************************************************************************/
	...
	class laes extends controller
	{
		...
		protected function index()
		{
			...
			$this->listaLaes = $laes->getLaes();
			$this->listaCiudades = $laes->getCiudades();
			$this->listaRoles = $laes->getRoles();

			$this->default_pw = _USER_DEFAULT_PASSW_;

			$this->addInReadyCode("
				MSLaes.init('"._MSFW_PATH_."', '"._MODEL_PATH_."', '"._VIEW_PATH_."', ".json_encode($this->listaCiudades).", ".json_encode($this->listaRoles).", '".$this->rol."');
				//
			");
		}
