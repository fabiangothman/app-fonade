...
window.MSDocumentos_emprendedor = (function(window, document, undefined) {
	...
	MSDocumentos_emprendedor.init = function(_path, _model, _view, _listaDocumentos, _listaProyectos, _user_id, _rol){
		path = _path;
		model = _model;
		view = path+_view;
        	...
        	success: function(response){ //una vez que el archivo recibe el request lo procesa y lo devuelve
        		...
        		if(response){
        			...
        			if(arrayListSend[3]==null || arrayListSend[3]=="null"){
        				...
        				$(tr_Obj).find("td[field=documento_id]").html($(tr_Obj).find("td[field=documento_id]").html()+" - <a href='"+path+"documentos/"+user_id+"/"+response+"' target='_blank' download>"+response+"</a>");
        			}
        		}else{
        	...
        	success: function(response){ //una vez que el archivo recibe el request lo procesa y lo devuelve
        		...
        		if(response){
        			...
        			if(arrayListSend[3]==null || arrayListSend[3]=="null"){
        				...
        				$(tr_Obj).find("td[field=documento_id]").html($(tr_Obj).find("td[field=documento_id]").html()+" - <a href='"+path+"documentos/"+user_id+"/"+objResponse.enlace+"' target='_blank' download>"+objResponse.enlace+"</a>");
        			}
        		}else{
